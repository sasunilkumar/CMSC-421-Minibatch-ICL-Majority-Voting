...
class MiniBatchDataset(Dataset):
    def _preprocess(self):
        print("Number of voters in dataset" + str(self.config.example_size) + "\n")
    ...
    def sample_batches(self, num_batches_to_sample):
        if num_batches_to_sample > len(self.batches):
            num_batches_to_sample = len(self.batches)
        return random.sample(self.batches, num_batches_to_sample)
